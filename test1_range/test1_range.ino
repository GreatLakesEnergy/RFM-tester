...
long t_this_packet, t_last_packet;
  ...
  if(NODEID==MASTER){
    ...
    if( Serial.available() > 0){
      ...
      switch(ABC){
        case 't':
            initialise_beacon( );
            Serial.readStringUntil('\n').toCharArray(buff, 100);
            //Serial.println(buff);
            test_duration = string_to_int( buff);
    ...
    if( STATE=='t' )
    {
       if(  test_length < test_duration )
       {
            ...
            if( radio.receiveDone() ) {
              ...
              if(STATE=='t')  test_length++;    // only count in t - stage
              
              if(CSV_OUTPUT){
                // CSV-format
                // "packet ID, variable name, variable data, ms since last packet"
                sprintf(buff,"[CSV]%d - %d,%s,%d,%d,%ld \n",
                              test_length, this_pack, variables[1],
                              string_to_int(variables[2]),
                               message.rssi, millis() );
                Serial.print(buff);             }
            
              // use ID to see if packet is missing
              packet_missing( this_pack );
              
              t_last_packet = t_this_packet;    // time
              
            }   // *** Eo receiveDone() 
            
        } // *** Eo if: test length < duration
        else {
          // * test duration ended
          
          if(STATE=='t'){
            // end test & print results
            STATE = 'i';
            
            
          }
          
        } // *** Eo else: test length = duration
